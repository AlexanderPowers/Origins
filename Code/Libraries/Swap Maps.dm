...
proc/SwapMaps_CreateFromTemplate(path, new_id)
	var/swapmap/M=new
	var/savefile/S
	var/text=0
	if(swapmaps_mode==SWAPMAPS_TEXT && fexists("[path].txt"))
		text=1
	else if(fexists("[path].sav"))
		S=new("[path].sav")
	else if(swapmaps_mode!=SWAPMAPS_TEXT && fexists("[path].txt"))
		text=1
	else
		world.log << "SwapMaps error in SwapMaps_CreateFromTemplate(): [path] file not found."
		return
	if(text)
		S=new
	...
	if(new_id)
		M.id = new_id
		swapmaps_loaded[M]=null
		swapmaps_byname[new_id]=M
	M.mode=text
	while(M.locked) sleep(1)
	return M
...
mob/SwapMapVerbs
	verb
		...
		BuildArena()
			set category = "Swap Maps"
			var/swapmap/M = SwapMaps_CreateFromTemplate("map_ForestBattlefield", "NewArena")
